...
module.exports = function(config) {
  ...
  if (!config.static) config.static = defaults.static;
  ...
  http.createServer(function(req, res) {
    if (req.url.match(/^\/$/)) {
      ...
      if (req.url.match(/-error-/gi)) {
        ...
        // set the req.url to the "original-url" if it's present in the header
        if (req.headers['original-url']) {
          req.url = req.headers['original-url'];
          if (req.url.match(/^http/i)) {
            // Remove any domain specific stuff, we are always assuming local files
            req.url = req.url.replace(/(https?:\/\/.*?)\//i, '/');
          }
        }
    ...
    // Enable this if we want to serve up static web content
      ...
      console.log('service static: ' + req.url);
